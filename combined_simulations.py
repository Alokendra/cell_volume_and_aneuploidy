...
def base_simulation(buckets):
    ...
    return read_out, sub_buckets


def core_sim_loop(repeats=5, buckets=[]):

    re_runs = []
    buckets = { key : [[]for _ in range(repeats)] for key in buckets }

    for i in range(0, repeats):
        read_out, sub_buckets = base_simulation(buckets)
...
if __name__ == "__main__":
    ...
    re_runs, buckets = core_sim_loop(buckets = [3, 15])
